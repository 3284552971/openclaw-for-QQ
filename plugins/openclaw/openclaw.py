...
DEFAULT_CONFIG = {
    ...
    "runtime": {
        ...
        "cli_agent": os.getenv("OPENCLAW_CLI_AGENT", "main"),
        ...
        "cli_stateless": False,
        # sessions_send: use gateway sessions_send with sessionKey (front-end visible)
        # agent_cli: use openclaw agent --session-id directly
        "session_backend": os.getenv("OPENCLAW_SESSION_BACKEND", "sessions_send"),
        # retry_then_cli: retry sessions_send, then fallback to CLI
        # error_only: no fallback, return error
        # direct_cli: skip sessions_send and use CLI directly
        "gateway_fallback_policy": os.getenv("OPENCLAW_GATEWAY_FALLBACK_POLICY", "retry_then_cli"),
        "gateway_retry_count": max(1, int(os.getenv("OPENCLAW_GATEWAY_RETRY_COUNT", "3"))),
        "gateway_retry_delay_seconds": max(0.0, float(os.getenv("OPENCLAW_GATEWAY_RETRY_DELAY_SECONDS", "1.0"))),
    },
    "bridge": {
        "enabled": True,
...
def _build_session_key(group_id, user_id, is_private):
    user = str(user_id).strip()
    if is_private:
        return f"qq:private:{user}:chat"
    group = str(group_id).strip()
    return f"qq:group:{group}:u:{user}:chat"


def _session_key(group_id, user_id, is_private):
    return _build_session_key(group_id, user_id, is_private)
...
def _extract_reply_and_usage(stdout_text):
    ...
    # Some OpenClaw runs only return status summaries like "completed"/"ok".
    # These are control statuses, not user-facing content.
    if summary.lower() in {"completed", "ok", "accepted", "success"}:
        return "", usage_total
...
def _is_control_noise_line(text: str) -> bool:
    ...
    if s in {"no_reply", "`no_reply`"}:
        return True
    if s in {"completed", "ok", "accepted", "success"}:
        return True
...
def start_bridge_server(bot):
    ...
    try:
        ...
        return
    # Clean stale task created from a closed/foreign loop, then recreate.
    if _BRIDGE_TASK:
        try:
            stale_loop = _BRIDGE_TASK.get_loop()
            stale = _BRIDGE_TASK.done() or stale_loop.is_closed() or (stale_loop is not loop)
        except Exception:
            stale = True
        if stale:
            _BRIDGE_TASK = None
    ...
    if task and not task.done():
        ...
        try:
            ...
            if not loop.is_closed():
                loop.create_task(_drain_task(task))
        except Exception:
            pass
...
def _resolve_session_backend(cfg):
    runtime_cfg = cfg.get("runtime") or {}
    backend = str(runtime_cfg.get("session_backend") or "").strip().lower()
    if backend in {"sessions_send", "agent_cli"}:
        return backend
    # Backward compatibility: old config without session_backend.
    return "sessions_send"


def _resolve_gateway_fallback_policy(cfg):
    runtime_cfg = cfg.get("runtime") or {}
    policy = str(runtime_cfg.get("gateway_fallback_policy") or "").strip().lower()
    if policy in {"retry_then_cli", "error_only", "direct_cli"}:
        return policy
    return "retry_then_cli"


def _gateway_retry_params(cfg):
    runtime_cfg = cfg.get("runtime") or {}
    retry_count = runtime_cfg.get("gateway_retry_count", OPENCLAW_MAX_RETRIES)
    retry_delay = runtime_cfg.get("gateway_retry_delay_seconds", OPENCLAW_RETRY_DELAY)
    try:
        retry_count = int(retry_count)
    except Exception:
        retry_count = OPENCLAW_MAX_RETRIES
    try:
        retry_delay = float(retry_delay)
    except Exception:
        retry_delay = OPENCLAW_RETRY_DELAY
    return max(1, retry_count), max(0.0, retry_delay)
...
async def _invoke_gateway_tool(cfg, tool_name, args, retries=1, retry_delay_seconds=None):
    runtime = _resolve_remote_runtime(cfg)
    endpoint = f"{runtime['url']}/tools/invoke"
    payload = {"tool": tool_name, "args": args}
    last_detail = ""
    last_kind = "failed"
    delay = OPENCLAW_RETRY_DELAY if retry_delay_seconds is None else max(0.0, float(retry_delay_seconds))
    ...
    for attempt in range(1, max(1, retries) + 1):
        ...
        if attempt < max(1, retries) and delay > 0:
            await asyncio.sleep(delay)

    return False, None, last_kind, last_detail


async def _run_openclaw_cli_once(cfg, session_key, prompt, with_session_id=True):
    runtime_cfg = cfg.get("runtime") or {}
    cli_cmd = str(runtime_cfg.get("cli_cmd") or "openclaw").strip() or "openclaw"
    cli_agent = str(runtime_cfg.get("cli_agent") or os.getenv("OPENCLAW_CLI_AGENT") or "main").strip() or "main"
    ...
    if with_session_id and str(session_key or "").strip():
        cmd_parts.append(f"--session-id {shlex.quote(str(session_key))}")
    elif not with_session_id:
        # OpenClaw CLI requires at least one target selector:
        # --to / --session-id / --agent
        cmd_parts.append(f"--agent {shlex.quote(cli_agent)}")
...
async def _run_openclaw_cli_with_retry(cfg, session_key, prompt):
    last_kind = "failed"
    last_detail = ""
...
async def _run_openclaw_cli_no_session_with_retry(cfg, prompt):
    last_kind = "failed"
    last_detail = ""
    for attempt in range(1, max(1, OPENCLAW_MAX_RETRIES) + 1):
        ok, details, error_kind, detail = await _run_openclaw_cli_once(
            cfg, "", prompt, with_session_id=False
        )
        if ok:
            return True, details, "", ""
        last_kind = error_kind or "failed"
...
def _extract_usage_total(usage):
...
def _payloads_to_reply(payloads):
    lines = []
    for payload in payloads:
        if not isinstance(payload, dict):
            continue
...
def _normalize_sessions_send_result(result):
    if not isinstance(result, dict):
        return False, None, "invoke_error", "invalid_response"

    details = result.get("details")
    if not isinstance(details, dict):
        details = {}
    status = str(details.get("status") or result.get("status") or "").strip().lower()
    error_text = str(details.get("error") or result.get("error") or "").strip()

    if "No session found" in error_text:
        return False, None, "session_missing", error_text
    if status == "timeout":
        return False, None, "timeout", error_text or "timeout"
    if status in {"error", "failed"} and error_text:
        return False, None, "invoke_error", error_text

    usage_total = None
    parts = []

    run_result = details.get("result")
    if isinstance(run_result, dict):
        payload_reply = _payloads_to_reply(run_result.get("payloads") or [])
        if payload_reply:
            parts.append(payload_reply)
        agent_usage = ((run_result.get("meta") or {}).get("agentMeta") or {}).get("usage") or {}
        usage_total = _extract_usage_total(agent_usage)
    ...
    if isinstance(content, list):
        for item in content:
            ...
            if isinstance(parsed, dict):
                ...
                parsed_error = str(parsed.get("error") or "").strip()
                if "No session found" in parsed_error:
                    return False, None, "session_missing", parsed_error
                if parsed_status in {"error", "failed"} and parsed_error:
                    return False, None, "invoke_error", parsed_error
                extracted_reply, extracted_usage = _extract_reply_and_usage(
                    json.dumps(parsed, ensure_ascii=False)
                )
                if extracted_reply:
                    parts.append(extracted_reply)
                if extracted_usage is not None:
                    usage_total = extracted_usage
            else:
                parts.append(text)
...
async def _run_openclaw_sessions_send_with_retry(cfg, session_key, prompt):
    retry_count, retry_delay = _gateway_retry_params(cfg)
    timeout_seconds = max(1, int(_resolve_remote_runtime(cfg)["timeout"]))
    # Newer OpenClaw builds require `task`; some older builds use `message`.
    send_variants = [
        {
    ...
    last_error_kind = ""
    last_detail = ""
    for args in send_variants:
        ok, result, error_kind, detail = await _invoke_gateway_tool(
            cfg,
            "sessions_send",
            args,
            retries=retry_count,
        ...
        if ok:
            return _normalize_sessions_send_result(result)
        last_error_kind, last_detail = error_kind, detail
        detail_text = str(detail or "").lower()
        # If this variant clearly mismatches required fields, try next variant.
        if "task required" in detail_text or "message required" in detail_text:
            continue
        # For non-parameter errors keep original behavior.
        return False, None, error_kind, detail
    return False, None, (last_error_kind or "failed"), (last_detail or "")


async def _run_openclaw_with_retry(cfg, session_key, prompt):
    backend = _resolve_session_backend(cfg)
    fallback_policy = _resolve_gateway_fallback_policy(cfg)

    if backend == "agent_cli":
        return await _run_openclaw_cli_with_retry(cfg, session_key, prompt)

    if fallback_policy == "direct_cli":
        return await _run_openclaw_cli_with_retry(cfg, session_key, prompt)

    ok, result, error_kind, detail = await _run_openclaw_sessions_send_with_retry(
        cfg, session_key, prompt
    )
    if ok:
        return True, result, "", ""
    if error_kind == "session_missing":
        return False, None, "session_missing", detail
    if fallback_policy == "retry_then_cli":
        _debug_log(
            "sessions_send_failure_fallback_cli "
            f"kind={error_kind or 'unknown'} detail={(detail or '')[:200]}"
        )
        ok_cli, result_cli, kind_cli, detail_cli = await _run_openclaw_cli_with_retry(
            cfg, session_key, prompt
        )
        if ok_cli:
            return True, result_cli, "", ""
        return False, None, (kind_cli or error_kind or "failed"), (detail_cli or detail or "")
    return False, None, (error_kind or "failed"), (detail or "")
...
def _legacy_gateway_slots(base_key, mode_tag):
    slots = []
    m_private = re.match(r"^qq:private:(.+):chat$", str(base_key or ""))
    if m_private:
        uid = m_private.group(1)
        slots.extend(
            [
                _gateway_session_slot(f"private:{uid}", mode_tag),
            ]
        )
    m_group = re.match(r"^qq:group:([^:]+):u:([^:]+):chat$", str(base_key or ""))
    if m_group:
        gid = m_group.group(1)
        uid = m_group.group(2)
        slots.extend(
            [
                _gateway_session_slot(f"group:{gid}:{uid}", mode_tag),
            ]
        )
    return slots
...
async def _ensure_gateway_session(cfg, state, base_key, mode_tag):
    ...
    # Lazy migrate from legacy slot names to the new canonical slot.
    if slot not in gateway_sessions:
        for legacy_slot in _legacy_gateway_slots(base_key, mode_tag):
            legacy_value = gateway_sessions.get(legacy_slot)
            if legacy_value is None:
                continue
            gateway_sessions[slot] = legacy_value
            break

    if backend == "sessions_send":
        # Canonical session key for sessions_send: front-end visible and stable.
        if str(gateway_sessions.get(slot) or "").strip() != str(base_key):
            gateway_sessions[slot] = str(base_key)
        for legacy_slot in _legacy_gateway_slots(base_key, mode_tag):
            if legacy_slot != slot:
                gateway_sessions.pop(legacy_slot, None)
        return True, slot, str(base_key), ""

    runtime_cfg = cfg.get("runtime") or {}
    cli_stateless = bool(runtime_cfg.get("cli_stateless", False))
    session_id = _get_session_id(state, base_key, reset=cli_stateless)
    gateway_sessions[slot] = session_id
    return True, slot, session_id, ""


async def _spawn_session_if_supported(cfg, session_key):
    # Different gateway versions have different sessions_spawn arg schemas.
    variants = [
        {"sessionKey": session_key},
        {"sessionKey": session_key, "task": "初始化会话"},
        {"sessionKey": session_key, "message": "初始化会话"},
    ]
    last_kind = ""
    last_detail = ""
    for args in variants:
        ...
        if not ok:
            last_kind, last_detail = error_kind, detail
            detail_text = str(detail or "").lower()
            if "task required" in detail_text or "message required" in detail_text:
                continue
        ...
        if not isinstance(details, dict):
            details = {}
        status = str(details.get("status") or result.get("status") or "ok").strip().lower()
        err_text = str(details.get("error") or result.get("error") or "").strip()
        if status in {"ok", "accepted", "created", "success"}:
            return True, "", ""
        if err_text and "already" in err_text.lower():
            return True, "", ""
        last_kind, last_detail = "invoke_error", (err_text or status or "spawn_failed")
    return False, (last_kind or "invoke_error"), (last_detail or "spawn_failed")
...
def _format_prompt(content, user_id, group_id, cfg, state, bridge_enabled):
    ...
    if bridge_enabled:
        ...
        bridge_info = "桥接工具当前不可用（/send_text、/file/read 等本地端点已禁用）。"
    values = {
        "USER_ID": user_id,
...
async def openclaw(group_id, user_id, bot, message):
    cfg = _load_config()
    session_backend = _resolve_session_backend(cfg)
    fallback_policy = _resolve_gateway_fallback_policy(cfg)
    gateway_retry_count, _gateway_retry_delay = _gateway_retry_params(cfg)
    bridge_enabled = bool((cfg.get("bridge") or {}).get("enabled", True))
    if bridge_enabled:
    ...
    _debug_log(
        ...
        f"session_backend={session_backend} fallback={fallback_policy}"
    )
    ...
    if content == "新建对话":
        ...
        if not ok and error_kind == "session_missing":
            if session_backend == "sessions_send":
                ok_spawn, spawn_kind, spawn_detail = await _spawn_session_if_supported(cfg, session_id)
                if ok_spawn:
                    ok, _result, error_kind, detail = await _run_openclaw_with_retry(cfg, session_id, "/new")
                elif spawn_kind == "tool_missing":
                    await _send_text(
                        bot,
                        group_id,
                        user_id,
                        "会话不存在，且网关未开放 sessions_spawn。请先发一条普通消息让会话自动初始化。",
                    )
                    return
                else:
                    hint = (spawn_detail or spawn_kind or "spawn_failed")[-300:]
                    await _send_text(bot, group_id, user_id, f"会话不存在且重建失败：{hint}")
                    return
            else:
                # CLI backend: recreate local session-id mapping.
                state = _load_state()
        ...
        if not ok and error_kind == "session_missing":
            await _send_text(bot, group_id, user_id, "会话不存在，请先发送一条普通消息初始化会话后再执行“新建对话”。")
            return
        if not ok and error_kind == "timeout":
            await _send_text(bot, group_id, user_id, f"已发送 /new，但执行超时（重试 {gateway_retry_count} 次）。")
            return
        if not ok:
    ...
    if session_backend == "sessions_send":
        runtime = _resolve_remote_runtime(cfg)
        _debug_log(
            f"sessions_send_call url={runtime['url']}/tools/invoke "
            f"session_key={session_id} timeout={runtime['timeout']}"
        )
    else:
        _debug_log(f"agent_cli_turn session_key={session_id}")
    ...
    if not ok and error_kind == "session_missing":
        if session_backend == "sessions_send":
            ...
            if ok_spawn:
                ...
                _debug_log(
                    "sessions_spawn_failed_try_cli_no_session "
                    f"kind={spawn_kind or 'unknown'} detail={(spawn_detail or '')[:200]}"
                )

            if not ok and error_kind == "session_missing":
                # Emergency fallback: some OpenClaw builds require pre-created
                # session index and may reject custom sessionKey permanently.
                # Fall back to CLI without session-id to keep user-facing reply available.
                ok_cli, result_cli, kind_cli, detail_cli = await _run_openclaw_cli_no_session_with_retry(
                    cfg, prompt
                )
                if ok_cli:
                    ok, result, error_kind, detail = True, result_cli, "", ""
                else:
                    await _send_text(
                        bot,
                        group_id,
                        user_id,
                        f"OpenClaw 会话不存在且重建失败：{(detail_cli or detail or spawn_detail or spawn_kind or 'unknown')[-500:]}",
                    )
                    return
        else:
            state = _load_state()
    ...
    if not ok and error_kind == "timeout":
        await _send_text(
            ...
            group_id,
            user_id,
            f"OpenClaw 响应超时，已重试 {gateway_retry_count} 次仍失败。",
        )
        return
    if not ok:
        ...
        await _send_text(
            ...
            group_id,
            user_id,
            f"OpenClaw 调用失败，已重试 {gateway_retry_count} 次：{detail}",
        )
        return
...
class OpenClawPlugin(NcatBotPlugin):
    ...
    async def on_load(self) -> None:
        # IMPORTANT:
        # ncatbot initializes plugins in a temporary thread event loop.
        # Do not start long-running asyncio servers here.
        pass
