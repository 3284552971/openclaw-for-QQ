...
- `sessions_send`：主链路走 `sessions_send(sessionKey=...)`，前端可按 QQ 对话维度看到会话
- `agent_cli`：主链路走 `openclaw agent --session-id`

### `runtime.gateway_fallback_policy`

- `retry_then_cli`：`sessions_send` 重试失败后自动回退 `agent_cli`
- `error_only`：只报错，不回退
- `direct_cli`：直接走 CLI，不走 `sessions_send`

`runtime.gateway_retry_count` 与 `runtime.gateway_retry_delay_seconds` 用于 `sessions_send` 重试参数。
...
- 当进入 CLI 无会话兜底时，使用 `openclaw agent --agent <cli_agent>` 执行
- 默认 `main`
...
若主链路使用 `openclaw agent`，在部分 OpenClaw 版本下可能被聚合到固定 `sessionKey`，导致前端只显示一条会话卡片。  
本插件默认改为 `runtime.session_backend: sessions_send`，并使用稳定 `sessionKey`（`qq:private:...` / `qq:group:...`）来保证前端可见性。

## 模式差异

- `sessions_send + retry_then_cli`：推荐，前端会话可见且故障可回退
- `agent_cli`：纯 CLI 链路，兼容性高，但前端会话可见性可能受 OpenClaw 版本影响
